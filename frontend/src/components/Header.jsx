...
import { 
  Music, Plus, User, LogOut, LayoutGrid, Menu, X, Sparkles, Send 
} from "lucide-react";
        ...
        <div className="flex items-center gap-4">
          {user ? (
            <div className="flex items-center gap-5">
              <Link to="/profile" className="p-1 rounded-full glass hover:bg-white/10 transition-colors border-white/5">
            ...
            <div className="hidden sm:flex items-center gap-4">
              <Link to="/login" className="px-6 py-2.5 text-[11px] font-black uppercase tracking-widest text-neutral-500 hover:text-white transition-colors">Identify</Link>
              <Link to="/register" className="px-7 py-3 rounded-2xl bg-white text-black text-[11px] font-black uppercase tracking-widest hover:bg-neutral-200 transition-all shadow-[0_8px_32px_rgba(255,255,255,0.1)]">Deploy Hub</Link>
            </div>
          )}
        ...
        <div className="p-8 space-y-3">
          ...
          {user ? (
            ...
            <div className="grid grid-cols-1 gap-4 pt-6">
              <Link to="/login" onClick={() => setMobileOpen(false)} className="p-5 text-center text-xs font-black uppercase tracking-[0.2em] border border-white/5 rounded-2xl">Access Identity</Link>
              <Link to="/register" onClick={() => setMobileOpen(false)} className="p-5 text-center text-xs font-black uppercase tracking-[0.2em] bg-white text-black rounded-2xl shadow-xl">Init Profile</Link>
            </div>
          )}
        </div>
      </div>
