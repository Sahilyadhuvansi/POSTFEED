...
import { Sparkles, Music, TrendingUp, Zap, Radio, Play, Headphones, Disc } from "lucide-react";
...
const AINexus = () => {
  ...
  const fetchRecommendations = useCallback(async () => {
    try {
      ...
      addToast(error.response?.data?.error || "AI uplink failed. Could not sync with Nexus.", "error");
    } finally {
      setLoading(false);
    }
  }, [mood, addToast]);
  ...
  const moods = [
    ...
    { id: "focus", label: "Singularity", icon: <TrendingUp className="w-4 h-4" /> },
    { id: "happy", label: "Euphoria", icon: <Sparkles className="w-4 h-4" /> },
    { id: "sad", label: "Melancholy", icon: <Headphones className="w-4 h-4" /> },
  ];
    ...
    <div className="mx-auto max-w-[1400px] px-6 pt-16 pb-24 min-h-screen">
      ...
      <div className="mb-20 border-b border-white/5 pb-10 flex flex-col md:flex-row md:items-end justify-between gap-10">
        <div className="space-y-4">
          <div className="flex items-center gap-3">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.4em] text-neutral-500">Neural Sync</p>
          </div>
          ...
          <p className="text-sm font-medium text-neutral-500 uppercase tracking-[0.2em] opacity-60">Architectural music picks tailored to your current frequency</p>
        </div>
      </div>

      {/* Mood Matrix */}
      <div className="mb-12">
        <p className="text-[10px] font-black text-neutral-600 uppercase tracking-widest mb-6 px-2">Frequency Filter</p>
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[...Array(6)].map((_, i) => (
            <div key={i} className="animate-pulse glass-dark h-32 rounded-[32px] border border-white/5" />
          ))}
        </div>
        ...
        <div className="flex flex-col items-center justify-center py-32 rounded-[48px] glass border-white/5">
          ...
          <p className="text-xl font-bold text-neutral-400 italic mb-2">Nexus is silent</p>
          <p className="text-xs font-black text-neutral-600 uppercase tracking-[0.2em]">Try adjusting your search frequency</p>
        </div>
      ) : (
              ...
              <div className="flex items-center gap-6">
                ...
                <div className="relative h-24 w-24 rounded-[32px] overflow-hidden shadow-2xl shrink-0 group-hover:rotate-3 transition-transform duration-700">
                    {music.thumbnailUrl || music.thumbnail ? (
                        <img src={music.thumbnailUrl || music.thumbnail} alt="" className="h-full w-full object-cover grayscale-[0.5] group-hover:grayscale-0 transition-all" />
                    ) : (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-1.5 overflow-hidden">
                    <h3 className="font-black text-xl text-white italic truncate tracking-tight">{music.title}</h3>
                  ...
                  <p className="text-xs font-bold text-neutral-500 uppercase tracking-widest mb-4">
                    Architect: <span className="text-white">@{music.userId?.username || "identity_unknown"}</span>
                  </p>
