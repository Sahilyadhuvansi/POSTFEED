...
import { X, Heart, MessageCircle, Share2, Eye, User, Calendar } from "lucide-react";
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        ...
        addToast("Failed to retrieve post. It may have been deleted.", "error");
        navigate("/");
      } finally {
        setLoading(false);
      }
      ...
      <div className="mx-auto max-w-[1400px] px-6 pt-24 pb-24">
        ...
        <div className="mb-12 flex items-center justify-between border-b border-white/5 pb-8">
            ...
            >
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.4em]">Return to stream</span>
            </button>
            <div className="hidden md:flex items-center gap-4 px-6 py-2 glass rounded-full border-white/5">
                <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
                <p className="text-[10px] font-black uppercase tracking-widest text-neutral-400">Deep Link Verified</p>
            </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_450px] gap-12 bg-neutral-900/20 rounded-[48px] border border-white/5 overflow-hidden p-4 lg:p-8 shadow-2xl">
          ...
          <div className="relative group rounded-[40px] overflow-hidden bg-neutral-900 border border-white/10 aspect-video lg:aspect-auto flex items-center justify-center min-h-[400px]">
            ...
            <div className="absolute bottom-8 left-8">
                <div className="glass px-6 py-3 rounded-2xl border-white/10 backdrop-blur-3xl flex items-center gap-3">
                    <Eye className="w-4 h-4 text-indigo-400" />
                    <span className="text-[10px] font-black uppercase tracking-[0.2em]">{Math.floor(Math.random() * 500) + 100} Focus Hits</span>
                </div>
            </div>
          </div>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                           ...
                           <p className="text-[9px] text-neutral-500 font-bold uppercase tracking-widest">Logged: {new Date(post.createdAt).toLocaleDateString()}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="relative z-10">
                    <div className="flex items-center justify-between mb-8">
                        <div className="flex items-center gap-8">
                            <button className="text-neutral-500 hover:text-white transition-all transform hover:scale-125 active:scale-90"><Heart className="w-7 h-7" /></button>
                            <button className="text-neutral-500 hover:text-white transition-all transform hover:scale-125 active:scale-90"><MessageCircle className="w-7 h-7" /></button>
                            <button className="text-neutral-500 hover:text-white transition-all transform hover:scale-125 active:scale-90"><Share2 className="w-7 h-7" /></button>
                        </div>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-indigo-500/5 border border-indigo-500/10 rounded-2xl p-4 text-center">
                            ...
                            <p className="text-xl font-black text-white">{Math.floor(Math.random() * 50)}%</p>
                        </div>
                        <div className="bg-pink-500/5 border border-pink-500/10 rounded-2xl p-4 text-center">
                            <p className="text-[9px] font-black text-pink-400 uppercase tracking-widest mb-1">Sentiment</p>
                            <p className="text-xl font-black text-white">POSITIVE</p>
                        </div>
                    </div>
                </div>
