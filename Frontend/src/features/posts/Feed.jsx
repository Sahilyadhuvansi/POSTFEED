...
import { MoreVertical, Trash2, Heart, MessageCircle, Share2, Eye, X, LayoutGrid, Sparkles, Send } from "lucide-react";
...
const Feed = () => {
  ...
  useEffect(() => {
    ...
    api.get(`/posts/feed?page=1&limit=${POSTS_PER_PAGE}`)
      ...
      .catch((err) => {
        addToast(err.response?.data?.error || "Neural link failed. Feed offline.", "error");
      })
    ...
    <div className="min-h-screen">
      <div className="mx-auto max-w-[1400px] px-6 pt-16 pb-24">
            ...
            <div className="flex items-center gap-6">
              <div className="relative group p-1 rounded-[32px] glass-dark border-white/5">
                <div className="h-20 w-20 rounded-[28px] overflow-hidden border border-white/10">
                  <img src={user.profilePic || DEFAULT_AVATAR} alt="" className="h-full w-full object-cover transition-all duration-700 group-hover:scale-125" />
                </div>
              ...
              <div>
                ...
                <p className="mt-2 text-xs font-black text-neutral-500 tracking-[0.3em] uppercase opacity-70">Neural hub status: Operational</p>
              </div>
            </div>
            <div className="flex gap-4">
              <div className="glass px-6 py-4 rounded-3xl border-white/5 text-center min-w-[120px]">
                <p className="text-[10px] font-black text-neutral-500 uppercase tracking-widest mb-1">Global Connect</p>
                <p className="text-2xl font-black text-white">{posts.length}<span className="text-xs text-indigo-400 ml-1">+</span></p>
              </div>
              <Link to="/create-post" className="flex items-center gap-3 glass-dark hover:bg-white text-white hover:text-black px-8 py-4 rounded-3xl border-white/5 transition-all active:scale-95">
        ...
        {posts.length === 0 ? (
          <div className="flex min-h-[50vh] flex-col items-center justify-center p-12 glass rounded-[40px] border-white/5 mt-8">
            ...
            <p className="text-sm font-medium text-neutral-500 mb-8 uppercase tracking-widest">Start the ripple in the stream</p>
            <Link to="/create-post" className="group relative overflow-hidden px-10 py-4 bg-white text-black text-xs font-black rounded-2xl transition-all hover:bg-neutral-200 uppercase tracking-widest flex items-center gap-3">
              Initial Expression
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
              ...
              >
                {post.image ? (
                  ...
                  <div className="h-full w-full flex items-center justify-center bg-white/[0.02] p-8 text-center transition-colors group-hover:bg-white/[0.05]">
                    <p className="text-sm text-neutral-400 font-bold leading-relaxed italic group-hover:text-white">"{post.caption}"</p>
                  </div>
                )}
                  ...
                  <div className="translate-y-8 group-hover:translate-y-0 transition-transform duration-700 delay-75 space-y-3">
                    <div className="flex items-center gap-3">
                      <div className="h-8 w-8 rounded-full border-2 border-white/20 overflow-hidden">
                        <img src={post.user?.profilePic || DEFAULT_AVATAR} alt="" className="h-full w-full object-cover" />
                      </div>
                      <p className="text-[10px] font-black text-white tracking-widest uppercase italic">@{post.user?.username || "identity_unknown"}</p>
                    </div>
                    <p className="text-[11px] text-neutral-400 font-medium line-clamp-2 leading-relaxed opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-200 lowercase italic tracking-tight">{post.caption}</p>
                  </div>
                </div>
            ...
            {hasMore && (
              <div ref={observerTarget} className="col-span-full py-24 flex flex-col items-center gap-6">
                ...
                <p className="text-[10px] text-neutral-600 font-black uppercase tracking-[0.5em] animate-pulse">Expanding Stream</p>
              </div>
            )}
          </div>
        )}
      ...
      {selectedPost && (
        ...
        >
          ...
          >
            <button className="absolute top-6 right-6 z-[70] p-3 glass border-white/10 text-white rounded-[20px] transition-all hover:bg-white hover:text-black hover:rotate-90" onClick={() => setSelectedPost(null)}>
              <X className="w-6 h-6" />
            </button>
            ...
            <div className="relative flex items-center justify-center bg-neutral-900/40 p-1">
              {selectedPost.image ? (
                <img src={selectedPost.image} alt="" className="w-full h-full object-contain sm:rounded-[40px] shadow-2xl" />
              ) : (
                <div className="p-20 text-center">
                  <p className="text-3xl font-black text-white italic tracking-tight leading-relaxed">"{selectedPost.caption}"</p>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col h-full bg-black border-l border-white/5">
              <div className="flex items-center justify-between p-8 border-b border-white/5">
                <div className="flex items-center gap-4">
                  <div className="h-12 w-12 rounded-2xl border-2 border-indigo-500/30 p-0.5 shadow-xl">
                    <img src={selectedPost.user?.profilePic || DEFAULT_AVATAR} alt="" className="h-full w-full rounded-xl object-cover" />
                  </div>
                  <div>
                    <p className="text-sm font-black text-white tracking-widest uppercase italic">@{selectedPost.user?.username || "Identity Unknown"}</p>
                    <div className="flex items-center gap-1.5 mt-1">
                        <div className="w-1.5 h-1.5 rounded-full bg-indigo-500" />
                        <p className="text-[10px] text-neutral-500 font-black uppercase tracking-[0.2em]">Primary Hub</p>
                    </div>
                  </div>
                </div>
                {user && String(user._id) === String(selectedPost.user?._id) && (
                  <div className="relative" ref={postMenuRef}>
                    <button onClick={(e) => { e.stopPropagation(); setActiveMenu(activeMenu ? null : selectedPost._id); }} className="p-3 rounded-2xl border border-white/5 bg-white/5 hover:bg-white/10 transition-colors text-neutral-400">
                    ...
                    {activeMenu === selectedPost._id && (
                      <div className="absolute right-0 top-full mt-3 w-56 rounded-3xl glass-dark border-white/10 shadow-2xl p-2.5 z-10 animate-fade-in-down">
                        <button onClick={(e) => handleDeletePost(e, selectedPost._id)} className="w-full flex items-center gap-4 px-5 py-4 text-xs font-black uppercase tracking-widest text-red-400 hover:bg-red-400/5 rounded-2xl transition-all">
              ...
              <div className="p-8 glass border-t border-white/5">
                <div className="flex items-center gap-6 mb-8">
                  <button className="text-neutral-500 hover:text-white transition-all transform hover:scale-125 active:scale-90"><Heart className="w-6 h-6" /></button>
                  <button className="text-neutral-500 hover:text-white transition-all transform hover:scale-125 active:scale-90"><MessageCircle className="w-6 h-6" /></button>
                  <button className="text-neutral-500 hover:text-white transition-all transform hover:scale-125 active:scale-90"><Share2 className="w-6 h-6" /></button>
                </div>
